...
class DokumenPegawaiController extends Controller
{
    public function index()
    {
        ...
        if (!$pegawai) {
            return view('pegawai.dokumen.index', ['dokumens' => collect([])]);
        }
        
        $dokumens = Dokumen::with(['folder', 'folder.kategoriFolder'])
            ->where('pegawai_id', $pegawai->id)
            ->where('status', 'aktif')
            ->latest()
            ->get()
            ->groupBy(function($item) {
                return $item->folder->kategoriFolder->nama ?? 'Lainnya';
            });
        
        return view('pegawai.dokumen.index', compact('dokumens'));
    }
    ...
    public function store(Request $request)
    {
        ...
        return redirect()->route('pengajuan.status')->with('success', 'Pengajuan upload berhasil dikirim');
    }
    ...
    public function ajukanHapus(Request $request, $id)
    {
        ...
        return redirect()->route('pengajuan.status')->with('success', 'Pengajuan hapus berhasil dikirim');
    }
}